...
| `rentometer_screener` | `GET /api/v1/atlas/screener` — find areas matching a multi-metric profile (several ranges at once) | 1 quickview |
...
## This package = local/stdio. Hosted connector = Rails `/mcp`.

This npm package is the **local stdio** server — for a developer wiring Rentometer into a
desktop/CLI MCP client (Claude Desktop, Cursor, Windsurf, Zed) with their own API key.

For **remote MCP connectors** (Claude / ChatGPT "add a custom connector"), point at the
**hosted endpoint served by the Rentometer Rails app at `https://www.rentometer.com/mcp`** — no
install, OAuth sign-in, per-user credits. Same 15 tools. Connecting is "paste the `/mcp` URL →
Connect → approve OAuth"; see the website's **Connect** page. (The hosted endpoint is part of the
Rails app, not this package.)
